...
# İşlenecek dosyaların listesi (Hem temiz hem gürültülü)
dosyalar = [
...
for dosya in dosyalar:
    ...
    try:
        ...
        # 1. Normal Durum (Temiz Su): pH değeri 7.2
        # Temiz veride gürültü istemiyorsak scale=0.0 yaparız, ama sensör doğası gereği ufak bir sapma (0.02) hep olur.
        df['pH_Level'] = np.random.normal(loc=7.2, scale=0.02, size=len(df))

        # 2. Sızıntı Anı (50. Saniye ve Sonrası): Gri su karışımı
        sizi_baslangici = 50.0
        sizi_indeksleri = df.index[df['Time'] >= sizi_baslangici].tolist()

        if sizi_indeksleri:
            # 7.2'den 8.5'e yavaş yavaş artan eğri
